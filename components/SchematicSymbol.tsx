...
export const SchematicSymbol: React.FC<Props> = ({ data, onClick, isSelected }) => {
  ...
  // Mentor Graphics / Xpedition Style Colors
  // Bright Yellow for unselected, Pink/Magenta for selected/hover
  const baseColor = 'var(--cad-wire)'; // Standard schematic yellow
  const highlightColor = 'var(--cad-select)'; // Selection pink
  const strokeColor = isSelected || isHovered ? highlightColor : baseColor;

  const fillColor = 'var(--cad-bg)'; // Black body
  const strokeWidth = isSelected || isHovered ? "2.5" : "1.5";
  const textColor = 'var(--cad-text)';
  const pinColor = 'var(--cad-pin-text)'; // Greenish for pins
  ...
  return (
    <g
      transform={`translate(${x}, ${y}) scale(${isHovered ? 2.0 : 1.8})`}
      {...interactionProps}
