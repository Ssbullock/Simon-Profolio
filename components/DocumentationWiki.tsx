...
export const DocumentationWiki: React.FC<Props> = ({ project, onBack, onClose, onNext }) => {
  ...
  const updateScrollFromMouse = (clientY: number) => {
    ...
    const targetScroll = percentage * (scrollRef.current.scrollHeight - scrollRef.current.clientHeight);
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-[#0a0a0a] text-gray-300 font-sans flex flex-col animate-in slide-in-from-right duration-300">
       ...
       <div className="flex-1 relative overflow-hidden">
          ...
          <div 
             ref={scrollbarRef}
             className="absolute right-6 top-1/2 -translate-y-1/2 h-[50vh] w-4 bg-[#181818] rounded-full border border-[#2a2a2a] shadow-[inset_0_2px_4px_rgba(0,0,0,0.5),0_4px_8px_rgba(0,0,0,0.5)] flex justify-center py-2 cursor-pointer z-50 group hover:border-[#444] transition-colors"
             onMouseDown={handleMouseDown}
          >
             <div className="w-[1px] h-full bg-[#333] relative group-hover:bg-[#444] transition-colors">
                <div 
                    className={`absolute w-2 h-2 bg-cad-accent rounded-full -left-[3.5px] shadow-[0_0_8px_rgba(0,122,204,1)] ${isDragging ? '' : 'transition-all duration-75 ease-out'}`}
