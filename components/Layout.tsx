...
export const Layout: React.FC<Props> = ({ 
  ...
  return (
    <div className="flex flex-col h-full w-full bg-cad-ui font-sans text-cad-text overflow-hidden print:h-auto print:overflow-visible">
      ...
      {/* Modern Toolbar */}
      <div className="hidden md:flex items-center gap-1 p-1 border-b border-cad-uiBorder bg-cad-uiDark print:hidden">
        <div className="flex gap-1 border-r border-cad-uiBorder pr-2">
            <IconButton 
                icon={<FloppyDisk size={20} />} 
                label="Save Resume (PDF)" 
                onClick={() => onSystemAction('Save')}
            />
            <IconButton 
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Sidebar Container */}
        <div className={`
            ${isSidebarOpen ? 'w-64 md:w-72 translate-x-0' : 'w-0 -translate-x-full opacity-0'} 
            flex-shrink-0 h-full z-20 bg-cad-ui border-r border-cad-uiBorder transition-all duration-300 
            absolute md:relative shadow-2xl md:shadow-none print:hidden
        `}>
            <div className="w-64 md:w-72 h-full relative">
                 {/* Sidebar Content */}
                 {sidebar}
        ...
        {/* Canvas */}
        <div className="flex-1 relative h-full bg-cad-bg shadow-inner z-10 print:w-full print:h-full print:bg-white">
            {children}
        </div>
      </div>
